...
class FreeplayMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		extras = new FlxSprite(scoreText.x + 50, 500).loadGraphic(Paths.image('menus/freeplay/extras'), true, 360, 110);
	...
	function weekIsLocked(name:String):Bool
	{
		...
		return (!leWeek.startUnlocked && leWeek.weekBefore.length > 0 && (!lucas.states.funkin.scripts.modes.StoryModeState.weekCompleted.exists(leWeek.weekBefore) || !lucas.states.funkin.scripts.modes.StoryModeState.weekCompleted.get(leWeek.weekBefore)));
	}
	...
	override function update(elapsed:Float)
	{
		...
		{
			scoreText.text = Language.getPhrase('personal_best', 'SCORE BEST: {1} ({2}%)', [lerpScore, ratingSplit.join('.')]);
			positionHighscore();
